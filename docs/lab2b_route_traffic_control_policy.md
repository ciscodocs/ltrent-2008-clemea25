# Lab 2b - Using Control Policy for Routing traffic from Stockholm to Sydney via London firewall in Same VRF
...
11. Now, click **Sequence Rule** and select **Match** to define the criteria for the control policy. In this step, we configure the policy to match routes based on a **specific site-id** and a **designated prefix-list**. For that we need to select **<font color="green">Site</font>** and **<font color="green">Prefix List</font>**. The **site-id** used in this control policy corresponds to **Sydney-Branch** (<font color="green">**site-id 20**</font>), ensuring the policy applies only to 
